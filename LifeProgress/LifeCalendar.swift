...
struct LifeCalendar: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            let cellSize = containerWidth / Double(LifeProgress.totalWeeksInAYear)
            ...
            ZStack(alignment: .topLeading) {
                ...
                Canvas { context, _ in
                    for yearIndex in 0 ..< progress.lifeExpectancy {
                        for weekIndex in 0 ..< LifeProgress.totalWeeksInAYear {
                            ...
                            if currentAge < progress.age {
                                context.fill(cellPath, with: .color(ageGroupColor))
                            } else if currentAge > progress.age {
                                context.fill(
                                    cellPath,
                ...
                ForEach(0 ..< LifeProgress.totalWeeksInAYear, id: \.self) { weekIndex in
                    ...
                    Rectangle()
                        .fill(weekIndex <= progress
                            .weekOfYear ? AgeGroup.getColorFor(age: progress.age + 1) :
            ...
            .frame(height: cellSize * Double(progress.lifeExpectancy))
            .onTapGesture {
